...
const unwarnHandler = async (ctx) => {
	...
	if (targets.length !== 1) {
		return ctx.replyWithHTML(
			'ℹ️ <b>Especifica un usuario para remover una advertencia.</b>',
	...
	if (allWarns.length === 0) {
		return ctx.replyWithHTML(
			html`ℹ️ ${link(userToUnwarn)} <b>, no tiene advertencias.</b>`,
		);
	}
	...
	if (!lastWarn) {
		return ctx.replyWithHTML(
			'❓ <b>404: Advertencia no encontrada.</b>',
		).then(scheduleDeletion());
	}
	...
	return ctx.loggedReply(html`
		❎ ${lrm}${ctx.from.first_name} <b>ha removido una advertencia a </b> ${link(userToUnwarn)}
