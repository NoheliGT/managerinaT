...
const nowarnsHandler = async (ctx) => {
	...
	if (warns.length === 0) {
		return ctx.replyWithHTML(
			html`ℹ️ ${link(userToUnwarn)} <b>, este usuario ya no tiene advertencias.</b>`,
		);
	}
	...
	✔️ ${lrm}${ctx.from.first_name} <b>ha liberado a </b> ${link(userToUnwarn)}
		de todas sus advertencias.
	`);
};
