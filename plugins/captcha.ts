	...
	await Promise.all(
		members.map(async (member) => {
			...
			const message = await ctx.replyWithHTML(
				`¡Bienvenido al grupo ${lrm}<a href="tg://user?id=${member.id}">${member.first_name}</a> [<code>${member.id}</code>] !\n` +
					'Por favor resuelve la siguiente operacion para ingresar al grupo en: ' +
					`${TIME_TO_ANSWER} segundos 🧭, de lo contrario seras <b>expulsado.</b> \n\n⏰ <i>Operación:</i> ` +
					`<b>${mathQuestion} = 🤔</b>`,
